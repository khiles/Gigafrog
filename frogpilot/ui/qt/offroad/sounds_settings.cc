...
FrogPilotSoundsPanel::FrogPilotSoundsPanel(FrogPilotSettingsWindow *parent, bool forceOpen) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> soundsToggles {
    ...
    {"SissyTauntVolume", tr("Sissy Mode Volume"), tr("<b>Set the volume for spoken \"Sissy Mode\" taunts.</b><br><br>These previously followed the automatic microphone-driven volume, which made them very quiet in a quiet cabin."), ""},
    {"GoodGirlVolume", tr("Good Girl Mode"), tr("<b>Volume of the lines spoken while the car is parked.</b>"), ""},
